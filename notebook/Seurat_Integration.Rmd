...
```
```{R}
FeaturePlot(coch.p15, features = "FOXG1")
```
...
for(gene in c("SOX9", "ATOH1", "GFI1", "POU4F3", "NOTCH1", "SLC1A3", "SOX2")){
  kern_avg = ksmooth(psd_t, coch.p8[["RNA"]][gene ,as.vector(filt)], kernel = "normal", bandwidth = 5)
  tt <- apply(psd_t,1,vec_pairwise_sub,kern_avg$x)
  colors <- rev(colorRampPalette(brewer.pal(11,'Spectral')[-6])(400))
  colors <- rev(viridis(400))
  log_col = log(rowSums(tt<1.25))
...
for(gene in c("SOX9", "ATOH1", "GFI1", "POU4F3", "NOTCH1", "SLC1A3", "SOX2")){
  kern_avg = ksmooth(psd_t, coch.p15[["RNA"]][gene ,as.vector(filt)], kernel = "normal", bandwidth = 5)
  tt <- apply(psd_t,1,vec_pairwise_sub,kern_avg$x)
  colors <- rev(colorRampPalette(brewer.pal(11,'Spectral')[-6])(400))
  colors <- rev(viridis(400))
  log_col = log(rowSums(tt<1.25))
